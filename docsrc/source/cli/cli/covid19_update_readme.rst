...
.. _covid19_update_readme:
	     
covid19_update_readme
======================
This CLI is designed for use by an external service to update the top-level ``README.rst`` with the *latest* COVID-19 cumulative cases and deaths for one of the top-N MSAs. It uses data encoded in |covid19_topN_json|, whether located in an external website or in a local directory, generated by :ref:`covid19_movie_updates`, and applies it to the template :download:`README_template.rst </_static/README_template.rst>`, to generate ``README.rst``.
...
* ``-j`` or ``--json`` optionally specifies the filename for |covid19_topN_json|; if it is specified, then use summary data in this file instead of from the external website. I developed this functionality because the web server updates were often too slow.
